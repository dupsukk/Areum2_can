...
class RobstrideMotor {
    ...
    template <RobstrideMotor_type Motor_type>
    RobstrideMotor<Motor_type>::RobstrideMotor(RobstrideMotor_type Motor_type, int can_interface,int can_id ): 
    ...
    bool RobstrideMotor::enable_motor( int motor_id) {
        uint32_t ext_id = (COMM_ENABLE << 24) | (HOST_ID << 8) | motor_id;
        return send_frame( ext_id, nullptr, 0);
    }
    
    bool RobstrideMotor::set_mode_raw(int motor_id, int8_t mode) {
    ...
    bool RobstrideMotor::write_limit(int motor_id, uint16_t param_id, float limit) {
        uint32_t ext_id = (COMM_WRITE_PARAMETER << 24) | (HOST_ID << 8) | motor_id;
        uint8_t data[8] = {0};
    ...
    bool RobstrideMotor::write_operation_frame(int motor_id, double pos, double kp_val, double kd_val) {
