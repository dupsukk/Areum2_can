...
void* update_Control_params(void* args){
    ...
    while(running){
        ...
        std::get<RS02_Vec>(Leg)[0].control_param.pos.store(std::clamp(ctrl_buf[0].pos, -1.0, 1.0), std::memory_order_relaxed);  // 라디안임.
        std::get<RS02_Vec>(Leg)[1].control_param.pos.store(std::clamp(ctrl_buf[1].pos, -1.0, 1.0), std::memory_order_relaxed);  // 나중에 하드코딩이 아니라 상수나 매크로로 바꿀 것.
        std::get<RS00_Vec>(Leg)[0].control_param.pos.store(std::clamp(ctrl_buf[2].pos, -1.0, 1.0), std::memory_order_relaxed);
        std::get<RS00_Vec>(Leg)[1].control_param.pos.store(std::clamp(ctrl_buf[3].pos, -1.0, 1.0), std::memory_order_relaxed);

        Feedback_Param fb_buf[4];
        fb_buf[0].pos = std::get<RS02_Vec>(Leg)[0].Feedback_param.pos.load(std::memory_order_relaxed);
        fb_buf[1].pos = std::get<RS02_Vec>(Leg)[1].Feedback_param.pos.load(std::memory_order_relaxed);
        fb_buf[2].pos = std::get<RS00_Vec>(Leg)[2].Feedback_param.pos.load(std::memory_order_relaxed);
        fb_buf[3].pos = std::get<RS00_Vec>(Leg)[3].Feedback_param.pos.load(std::memory_order_relaxed);
        ...
        /*
            ...
            (..., [&](auto& vec) {
                //for (auto& motor : vec) motor.write_updated_operation_frame();
                for (auto& motor : vec) motor.control_param.pos = std::clamp(shm_ptr->ctrl.data[idx++].pos , -10.0 , 10.0); // 와 이건 일반화 하는게 독인듯. TODO : 수동 언롤링 하기 
            }(vecs));
        }, Leg);
        */
       usleep(1000);
